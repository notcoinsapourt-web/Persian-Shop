...
export default function CatalogPanel({
  ...
  const filtered = useMemo(() => {
    ...
    let list = products.filter(product => (category === "all" || product.category === category) && (!needle || `${product.name} ${product.en} ${product.description}`.toLowerCase().includes(needle)));
  ...
  return (
    <div className="catalog-layer" role="dialog" aria-modal="true" aria-label="دسته‌بندی محصولات">
      ...
      <div className="page-container catalog-search-row">
        <label className="catalog-search"><Search size={20}/><input value={query} onChange={event => onQuery(event.target.value)} placeholder="جستجو در محصولات..."/>{query && <button onClick={() => onQuery("")} aria-label="پاک کردن"><X size={16}/></button>}</label>
      </div>
      ...
      <div className="page-container catalog-layout">
        ...
        <main className="catalog-content">
          <div className="catalog-content-header">
            <div><h1>{selectedCategory?.name || "همه محصولات"}</h1><p>{filtered.length} محصول برای نمایش</p></div>
          ...
          <div className="catalog-filter-summary"><SlidersHorizontal size={16}/><span>{category === "all" ? "همه دسته‌ها" : shortCategory(selectedCategory?.name || "")}</span>{query && <span>جستجو: «{query}»</span>}</div>
